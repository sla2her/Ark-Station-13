...
/datum/controller/subsystem/communications/proc/make_announcement(mob/living/user, is_silicon, input, syndicate, list/players)
	if(!can_announce(user, is_silicon))
		return FALSE
	if(is_silicon)
		minor_announce(html_decode(input),"[user.name] Announces:", players = players, sound_override = 'modular_arkstation/modules/sounds-renewal/sound/announce/ai_tone.ogg') // Ark Station 13 Edit
